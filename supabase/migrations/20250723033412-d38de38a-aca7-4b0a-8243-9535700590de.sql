...
-- Criar tabela de configurações Stripe
...
INSERT INTO public.billing_plans (name, description, price, interval, max_users, features) 
SELECT 'Básico', 'Plano básico para pequenos negócios', 29.90, 'month', 3, '["Gestão de vendas", "Controle de estoque básico", "Emissão de NFCe"]'
WHERE NOT EXISTS (SELECT 1 FROM public.billing_plans WHERE name = 'Básico');

INSERT INTO public.billing_plans (name, description, price, interval, max_users, features) 
SELECT 'Econômico', 'Plano intermediário com mais funcionalidades', 59.90, 'month', 10, '["Gestão de vendas", "Controle de estoque avançado", "Emissão de NFe/NFCe", "Relatórios básicos"]'
WHERE NOT EXISTS (SELECT 1 FROM public.billing_plans WHERE name = 'Econômico');

INSERT INTO public.billing_plans (name, description, price, interval, max_users, features) 
SELECT 'Completo', 'Plano completo para empresas em crescimento', 99.90, 'month', 25, '["Todas as funcionalidades", "Integrações com marketplaces", "Relatórios avançados", "Suporte prioritário"]'
WHERE NOT EXISTS (SELECT 1 FROM public.billing_plans WHERE name = 'Completo');

INSERT INTO public.billing_plans (name, description, price, interval, max_users, features) 
SELECT 'Exclusivo', 'Plano premium com recursos ilimitados', 199.90, 'month', -1, '["Recursos ilimitados", "Whitelabel", "API personalizada", "Suporte dedicado", "Customizações"]'
WHERE NOT EXISTS (SELECT 1 FROM public.billing_plans WHERE name = 'Exclusivo');