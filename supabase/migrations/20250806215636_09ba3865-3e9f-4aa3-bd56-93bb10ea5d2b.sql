-- Fix user data and create safe RLS policies
-- This migration creates the missing user profile and company association with proper document format
...
-- Get the operador access level ID and create user data
DO $$
DECLARE
...
BEGIN
  ...
  -- Create profile for the user if it doesn't exist with proper CNPJ format
  ...
  SELECT 
    ...
    '01.224.117/0001-11', -- Proper CNPJ format
    '61983496459',
  ...
  -- If no company exists, create one with proper CNPJ format
...
-- Replace the policies with completely safe ones that don't use subqueries
DROP POLICY IF EXISTS "safe_user_companies_select" ON public.user_companies;
DROP POLICY IF EXISTS "safe_user_companies_insert" ON public.user_companies;
DROP POLICY IF EXISTS "safe_user_companies_update" ON public.user_companies;
DROP POLICY IF EXISTS "safe_user_companies_delete" ON public.user_companies;

-- Create ultra-safe policies that definitely won't cause recursion
CREATE POLICY "ultra_safe_user_companies_select" 
ON public.user_companies FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "ultra_safe_user_companies_insert" 
ON public.user_companies FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "ultra_safe_user_companies_update" 
ON public.user_companies FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY "ultra_safe_user_companies_delete" 
ON public.user_companies FOR DELETE 
USING (false); -- Only admins can delete, handle via app logic

-- Also create basic profile policies
