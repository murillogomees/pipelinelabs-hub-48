...
-- RLS policies for tables with direct company_id
...
-- Apply same pattern to tables with direct company_id
DO $$
DECLARE
  table_name TEXT;
  tables TEXT[] := ARRAY[
...
-- Special RLS policies for sale_items (accessed through sales.company_id)
CREATE POLICY "Company scoped select sale_items" ON public.sale_items
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Company scoped insert sale_items" ON public.sale_items
...
CREATE POLICY "Company scoped update sale_items" ON public.sale_items
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM public.sales 
      WHERE sales.id = sale_items.sale_id 
...
CREATE POLICY "Company scoped delete sale_items" ON public.sale_items
  FOR DELETE USING (
    EXISTS (
      SELECT 1 FROM public.sales 
      WHERE sales.id = sale_items.sale_id 
